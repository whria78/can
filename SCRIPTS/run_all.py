...
parser.add_argument('--repeat', type=float, default=20, help='repeat experiment by (20 by default) times')
...
#### RUN
for i in range(0,parser.repeat):
    for select_train in train_list:
        for select_test in public_list:
            print(select_train,select_test)
            cmd_=f"python3 train.py --model efficientnet --opt radam --batch {parser.batch} --epoch {parser.epoch} --lr {parser.lr} --train '{select_train}' --test {select_test} --result {parser.result_file}"
            print(cmd_)
            os.system(cmd_)
            time.sleep(1)
